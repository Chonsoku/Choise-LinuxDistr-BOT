...
async def get_distribution_recommendation(level: str, answers: dict) -> str:
    ...
    if level == "beginner":
        # prompt_for_name = f"""Ты дружелюбный и приветливый эксперт по Linux. На основе ответов НОВИЧКА (новичок), который хочет перейти на Linux, 
        # дай ему персонализированную рекомендацию дистрибутива. Порекомендуй ТОЛЬКО дружественные новичкам дистрибутивы: Ubuntu, Linux Mint, Zorin OS, Pop!_OS, Fedora, elementary OS.
        # Не предлагай Arch, Gentoo, Void и другие сложные дистрибутивы.
        # На основе ответов пользователя, напиши ТОЛЬКО НАЗВАНИЕ дистрибутива Linux. Твой выбранный дистрибутив должен сооствествовать твоему следующему сообщению!
        
        # ОТВЕТЫ ПОЛЬЗОВАТЕЛЯ: {answers_text}
        
        # Правила:
        # - Напиши только 1 слово — название дистрибутива
        # - Никаких ASCII-артов
        # - Никаких объяснений
        # - Никаких эмодзи
        ...
        1. Дай небольшое описание дистрибутива (2-3 предложения)
        2. Объясни, ПОЧЕМУ именно этот дистрибутив подходит этому пользователю (опираясь на его ответы)
        ...
        # prompt_for_name = f"""Ты эксперт по Linux с глубокими знаниями популярных и редких дистрибутивов.  
        # На основе ответов пользователя, напиши ТОЛЬКО НАЗВАНИЕ дистрибутива Linux. Твой выбранный дистрибутив должен сооствествовать твоему следующему сообщению!
